...
export class PrReviewProcessor extends WorkerHost {
  ...
  async process(job: Job<PrReviewJobData>): Promise<void> {
    ...
    const review = await this.reviewsRepo.createPending(data);
    ...
    try {
      ...
      if (diffChunks[0].files.length === 0) {
        ...
        await this.reviewsRepo.markNoContent(review.id);
        return;
      }
      ...
      await this.reviewsRepo.saveCompleted(review.id, mergedResult, provider,'',1);

      // 7. Post review comment on GitHub PR
      const [owner2, repo2] = data.repoFullName.split('/');
      await this.commentService.postReview(data, mergedResult, owner2, repo2);
      ...
      await this.reviewsRepo.markFailed(review.id, (err as Error).message);
  ...
  onFailed(job: Job, err: Error) {
    this.logger.error(
      { jobId: job.id, pr: job.data.prNumber, err: err.message },
      '❌ Job failed',
    );
  }
}
